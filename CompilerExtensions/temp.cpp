...
#include "../cmake-build-debug/_deps/clang-src/include/clang-c/Index.h"
#include <ctype.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
        ...
        if (!clang_equalCursors(Referenced, clang_getNullCursor())) {
            ...
            if (clang_getCursorKind(Referenced) == CXCursor_TypedefDecl) {
                ...
                if (clang_Type_isTransparentTagTypedef(T)) {
                    ...
                    while(T.kind == CXType_Typedef){
                        T = clang_getTypedefDeclUnderlyingType(clang_getTypeDeclaration(T));
                    }
                    CXString S = clang_getTypeSpelling(T);
                    printf(" (Typedef: %s)", clang_getCString(S));
                    clang_disposeString(S);
                }
            }
            if (clang_getCursorKind(Referenced) == CXCursor_ClassDecl) {
...
int perform_test_reparse_source(int argc, const char **argv, int trials,
   ...
   * files, so that we have a way to test results after changing the source. */
    ...
    for (trial = 0; trial < trials; ++trial) {
        ...
        Err = static_cast<CXErrorCode>(clang_reparseTranslationUnit(
                TU,
...
int perform_code_completion(int argc, const char **argv, int timing_only) {
    ...
    Err = static_cast<CXErrorCode>(clang_reparseTranslationUnit(TU, 0, 0,
                                                                clang_defaultReparseOptions(TU)));
...
static ImportedASTFilesData *importedASTs_create(void) {
    ...
    p = static_cast<ImportedASTFilesData *>(malloc(sizeof(ImportedASTFilesData)));
    assert(p);
    p->filenames = static_cast<char **>(malloc(MAX_IMPORTED_ASTFILES * sizeof(const char *)));
    assert(p->filenames);
    p->num_files = 0;
    return p;
}
...
static int index_compile_args(int num_args, const char **args,
    ...
    if (result != CXError_Success)
        describeLibclangFailure(static_cast<CXErrorCode>(result));
...
void thread_runner(void *client_data_v) {
    thread_info *client_data = static_cast<thread_info *>(client_data_v);
